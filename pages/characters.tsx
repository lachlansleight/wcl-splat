...
const CharactersPage = (): JSX.Element => {
    ...
    const processConsumes = useCallback(async () => {
        ...
        for(let i = 0; i < logData.processedReport.characters.length; i++) {
            ...
            const consumes = (await axios.post(`/api/consumes?apiKey=${config.apiKey}&player=${character.name}`, {report: logData.processedReport})).data;
            setLogData(cur => ({...cur, consumeData: {...cur.consumeData, [character.name]: consumes[character.name]}}));
            setLoadingIndex(i + 1);
    ...
    const processBombs = useCallback(async () => {
        ...
        for(let i = 0; i < logData.processedReport.fights.length; i++) {
            setLoadingIndex(i + 1);
            const bombs = (await axios.post(`/api/bombs?apiKey=${config.apiKey}&fightIndex=${i}`, {report: logData.processedReport})).data;
                ...
                <div className="flex justify-center gap-4 mb-4">
                    {logData.processedReport 
                        ? <Button className="bg-red-400" onClick={() => {
                            if(!window.confirm("Really clear data? You will need to re-process to recover it.")) return;
                            setLogData(cur => ({...cur, processedReport: undefined, consumeData: undefined, bombData: undefined}))
                        }}>Clear Fights</Button>
                        : <Button onClick={processReport}>Process Fights</Button>
                    }
                    {logData.consumeData 
