...
const ConsumesPage = (): JSX.Element => {
    ...
    const [loadingPhase, setLoadingPhase] = useState<"consumes" | "bombs">("consumes");
    ...
    const processConsumes = useCallback(async () => {
        ...
        const engineerCount = logData.processedReport.characters.reduce((acc, cur) => acc + (cur.isEngineer ? 1 : 0), 0);
        ...
        for(let i = 0; i < logData.processedReport.characters.length; i++) {
            ...
            const consumes = (await axios.post(`/api/consumes?apiKey=${config.apiKey}&player=${character.name}`)).data;
