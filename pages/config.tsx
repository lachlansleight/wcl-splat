...
const ConfigPage = (): JSX.Element => {
    ...
    const runTest = useCallback(async () => {
        ...
        setTestResult("");
        try {
            const response = (await axios(`/api/report?apiKey=${config.apiKey}&reportId=4ZdWAv6FgrzwnkpT`)).data;
        ...
        <Layout>
            <div className="grid place-items-center h-96">
                <div className="flex flex-col gap-4 w-1/2 justify-center">
                    ...
                    <p>To obtain one, go to <a href="https://classic.warcraftlogs.com/profile" target="_blank" rel="noreferrer">your warcraftlogs profile page</a>, scroll to the bottom and follow the instructions.</p>
                    <p>Copy-and-paste your API key into the text box below. It will be saved to your browser&apos;s local storage so you don&apos;t have to paste it every time.</p>
                    <TextField label="WarcraftLogs API Key" value={config.apiKey} onChange={v => setConfig(cur => ({...cur, apiKey: v}))} />
                    {loading ? (
                    ...
                    {testResult.includes("Error") 
                        ...
                        : !!testResult ? <p className="text-green-300 text-center">{testResult}</p>
                        : <p>&nbsp;</p>
                    }
                </div>
            </div>
