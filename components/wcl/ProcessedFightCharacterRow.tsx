...
const ProcessedFightCharacter = ({
    ...
    return (
        <div className="flex items-center gap-4">
            <div
                className="text-black py-1 px-2 leading-none w-72 rounded"
                style={{
            ...
            >
                ...
                <span>
                    {character.specs[0]}{" "}
                    {character.class === "DeathKnight" ? "DK" : character.class}
                </span>
            </div>
            <div className="w-4" />
            {character.mainRole === "tank" && <div className="w-20" />}
            {character.mainRole === "dps" && (
                <Pill className="border-white w-20">{getShortNumber(damageDone, 1)}</Pill>
            )}
            {character.mainRole === "healer" && (
                <Pill className="border-white w-20">{getShortNumber(healingDone, 1)}</Pill>
            )}
            <div className="w-4" />
            {consumes && (
                <>
                    <Pill
                        className="border-white w-12"
                        percentage={consumes.prePot}
                        percentageOpacity="40"
                    >
                        {Math.round(consumes.prePot * 100)}%
                    </Pill>
                    <Pill
                        className="border-white w-12"
                        percentage={consumes.fightPot}
                        percentageOpacity="40"
                    >
                        {Math.round(consumes.fightPot * 100)}%
                    </Pill>
                    <Pill
                        className="border-white w-12"
                        percentage={consumes.foodPercentage}
                        percentageOpacity="40"
                    >
                        {Math.round(consumes.foodPercentage * 100)}%
                    </Pill>
                    {consumes.usesElixirs ? (
                        <Pill
                            className="border-white w-12"
                            percentage={consumes.dualElixirPercentage}
                            percentageOpacity="40"
                        >
                            {Math.round(consumes.dualElixirPercentage * 100)}%
                        </Pill>
                    ) : (
                        <Pill
                            className="border-white w-12"
                            percentage={consumes.flaskPercentage}
                            percentageOpacity="40"
                        >
                            {Math.round(consumes.flaskPercentage * 100)}%
                        </Pill>
                    )}
                </>
            )}
